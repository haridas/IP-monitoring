...
try:
    import ipchecker
    from mailing.mail_format import get_report
    from mailing.smtp import Smtp
except:
...
try:
    ...
    print "Missing iniparser module. Please install it."
    print  sys.exc_info()[1]
    sys.exit(0)
'''
...
class ipmonitoring:
        ...
        if ip == '':
            ...
            if len(ip) ==1:
                ...
                for i in range(rang):
                    ...
                    if flag == 0:
                        temp_ip[3]=str(int(temp_ip[3]) + 1); #---->to int ---> Increment last digit ----> string.
    ...
    def main(self):
        ...
        '''
        Main function which interface and transfer the controls to differnt modules.
        Responsibilties:-
        ---------------
        1.Read configurations files from the ipmonitoring.ini.
        2.Properly arrange the inputs from configurations into the compatible formats.
        ...
        try:
            ...
            print '''
                    ...
                    Use this format :  ipmonitoring -[d|h|s|all] [ip,ip/cidr,...] [email,emails...]
        ...
        if status_flag > 0:
            ...
            try:
                ...
                #Use render() function to get the string output, required to wirte to report html.
	    ...
	    check_time = str(localtime[0])+'-'+str(localtime[1])+'-'+str(localtime[2])
            mail_subject = 'Ip Status Report from Sparksupport['+ check_time +']';
            
	    try:
                mail=Smtp('localhost','','')  #Setting the local default MTA to send mails, from current script running user.
		...
		log = ", ".join(self.email_list) + " | " + check_time + " " + str(localtime[3])+":"+str(localtime[4])+":"+str(localtime[5]) + " | " + sys.argv[2]
                file.write(log)
                file.write("\n")
                file.close()
            except:
    ...
    def read_commandline(self):
            ...
            if sys.argv[1][0] == '-':
                ...
                self.dnsbl_check = self.honeypot_check = self.senderbase_check = 'True'
